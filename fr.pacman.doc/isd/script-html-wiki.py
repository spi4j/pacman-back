...
# Nettoyage des lien hypertext
def auto_linkify(lines):
    url_pattern = re.compile(r'(?<!\()(?P<url>https?://[^\s<>\)]+)')
    linked_lines = []
    for line in lines:
        # Ne pas toucher aux lignes déjà contenant un lien Markdown ou dans un bloc de code
        if line.strip().startswith('    ') or '[' in line:
            linked_lines.append(line)
        else:
            # Remplacer l'URL brute par un lien Markdown
            linked_line = url_pattern.sub(r'[\g<url>](\g<url>)', line)
            linked_lines.append(linked_line)
    return linked_lines

# Gestion des blocs [!NOTES] [!TIPS] etc...   
    ...
    for line in lines:
        ...
        if stripped.startswith('> [!') and stripped.endswith(']'):
            if in_admonition:
                ...
                icon = ICON_MAP.get(admonition_type, '')
                result.append(f'<div class="admonition {admonition_type}">')
                result.append(f'<p class="admonition-title">{icon} {admonition_type.capitalize()}</p>')
                result.append(html_content)
                result.append('</div>')
                content_buffer = []

            admonition_type = stripped[4:-1].lower()
            in_admonition = True
            content_buffer = []
        elif in_admonition and stripped.startswith('>'):
            content_buffer.append(stripped[1:].lstrip())
        else:
            if in_admonition:
                md_content = '\n'.join(content_buffer)
                html_content = markdown(md_content, extensions=['fenced_code', 'tables'])
...
def main():
    ...
    for i, chap in enumerate(chapters):
        ...
        md_lines = auto_linkify(md_lines) 
        md_lines = parse_admonitions(md_lines)
