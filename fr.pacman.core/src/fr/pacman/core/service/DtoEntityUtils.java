...
public final class DtoEntityUtils {
	...
	public static List<Attribute> get_attributes(final DTO p_dto) {
		...
		List<Attribute> allAttributes = p_dto.getAssociatedTypes().stream().map(StructuredType::getAttributes)
				.flatMap(EList::stream).collect(Collectors.toList());
		allAttributes.addAll(p_dto.getAttributes());
	...
	public static List<Attribute> get_attributes(final Entity p_entity) {
		List<Attribute> allAttributes = p_entity.getAssociatedTypes().stream().map(StructuredType::getAttributes)
				.flatMap(EList::stream).collect(Collectors.toList());
		allAttributes.addAll(p_entity.getAttributes());
		return allAttributes;
	}
